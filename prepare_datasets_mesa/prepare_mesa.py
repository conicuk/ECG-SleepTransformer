...
def generate_linear_spectrogram(data, n_fft=254, hop_length=274):
...
def main():
    ...
    parser.add_argument("--original_data_dir", type=str, default="./mesa_ECG_class4", #<- save directory name
                        help="Directory to save the processed data files.")
    parser.add_argument("--spectrogram_dir", type=str, default="./mesa_ECG_mel_class4", #<- save directory name
    ...
    for file_id in range(len(edf_fnames)):
        ...
        total_epoch_len = EPOCH_SEC_SIZE * int(sampling_rate)
        valid_len = (len(raw_ch) // total_epoch_len) * total_epoch_len
        raw_ch = raw_ch[:valid_len]

        n_epochs = len(raw_ch) // total_epoch_len
        if len(raw_ch) % (EPOCH_SEC_SIZE * sampling_rate) != 0:
            print(f"❌ Length issue in {edf_fnames[file_id]} — skipped.")
        ...
        for epoch_index, epoch in enumerate(x):
            ...
            #Sxx = generate_linear_spectrogram(epoch.flatten()) #linear spectrogram

            #current_label = y[epoch_index]
            # if current_label not in class_processed:
            #     visualize_spectrogram(Sxx, current_label, f'MESA_{file_id}_{current_label}')
            #     visualize_ecg(epoch, current_label, f'MESA_{file_id}_{current_label}')
